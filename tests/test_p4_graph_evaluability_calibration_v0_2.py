...
RUNNER_PATH = ROOT / "scripts" / "run_p4_graph_evaluability_calibration_v0_2.py"
SPEC = importlib.util.spec_from_file_location(
    "p4_graph_evaluability_runner_v0_2", RUNNER_PATH
)
...
SELECTOR_ONLY_MEASUREMENT_KEYS = {
    "lexicographic_objective",
    "jaccard_used_as_objective",
}
SELECTOR_MEASUREMENT_KEYS = (
    CONFIRMATION_MEASUREMENT_KEYS | SELECTOR_ONLY_MEASUREMENT_KEYS
)
...
def _disjoint_edges() -> tuple[frozenset[tuple[int, int]], ...]:
    ...
    return tuple(
        frozenset(all_edges[index * 147 : (index + 1) * 147]) for index in range(3)
    )
...
def _matrix_projection(
    ...
    *,
    ...
    selected_edge_order = (
        {item["family"]: item["edge_fingerprint_sha256"] for item in selected}
...
def _candidate_projection(
    ...
    *,
    ...
    if edge_count != 147:
        all_edges = [
            (left, right) for left in range(49) for right in range(left + 1, 49)
        ]
        path = [(row, row + 1) for row in range(48)]
        canonical_edges = sorted(
            path + [edge for edge in all_edges if edge not in path][: edge_count - 48]
        )
    ...
    return {
        ...
        "graph_input_fingerprint_sha256": digest,
        "vertex_order_sha256": P4.array_sha256(np.arange(49, dtype="<i8")),
        "state_sha256": digest,
        "specification_fingerprint_sha256": digest,
        "family_identity_fingerprint_sha256": digest,
        "graph_fingerprint_sha256": digest,
...
def _base_triplet_measurements_projection(
    ...
    pairwise = [
        {
            ...
            "intersection_count": len(
                set(reconstructed[left]["edges"]) & set(reconstructed[right]["edges"])
            ),
            "union_count": len(
                set(reconstructed[left]["edges"]) | set(reconstructed[right]["edges"])
            ),
    ...
    return {
        ...
        "component_count_sum": sum(int(item["component_count"]) for item in selected),
        "pairwise": pairwise,
...
def _selector_triplet_measurements_projection(
    selected: list[dict[str, object]],
) -> dict[str, object]:
    measurements = _base_triplet_measurements_projection(selected)
...
def _selector_projection(*, passing: bool = True) -> dict[str, object]:
    ...
    measurements = (
        _selector_triplet_measurements_projection(selected) if passing else None
    )
    return {
        "projection_schema_version": "spirallens.p4-selector-projection.v0.2",
...
def _eligible_alternate_selector(
    ...
    selected[0]["graph_fingerprint_sha256"] = P4.canonical_json_sha256(selected[0])
    selected[1]["graph_fingerprint_sha256"] = P4.canonical_json_sha256(selected[1])
    measurements = _selector_triplet_measurements_projection(selected)
...
def _install_synthetic_selector_recompute(
    monkeypatch: pytest.MonkeyPatch,
) -> dict[str, object]:
    selector = _selector_projection()
    monkeypatch.setattr(
        P4,
        "_recompute_calibration_selector",
        lambda _protocol: types.SimpleNamespace(projection=selector),
    )
    return selector
...
def _confirmation_structural(
    ...
    *,
    ...
    return {
        ...
        "selected": selected,
        "triplet_measurements": _base_triplet_measurements_projection(selected),
        ...
        [
            {
                ...
                "state": "pass",
                "confirmation_structural_sha256": P4.canonical_json_sha256(structural),
            }
        ],
    ...
    for control_id, field in invariant_fields.items():
        ...
        for family in P4.FAMILY_ORDER:
            ...
            if control_id == "joint_vertex_permutation":
                ...
                transformed_content = P4._row_edge_content_sha256(transformed_edges)
                vertex_evidence = {}
            ...
            observations.append(
                {
                    ...
                    **vertex_evidence,
                    ...
                    "base_edge_order_sha256": selected_item["edge_fingerprint_sha256"],
        ...
        [
            {
                ...
                "cycle_rank": 0,
                "edge_fingerprint_sha256": collapsed_graph["edge_fingerprint_sha256"],
...
def _full_attempted_result(
    *,
    ...
    selector = _selector_projection()
    calibration = _matrix_projection("calibration", selected=selector["selected"])
    structural = _confirmation_structural(
        selector, edge_shift_offset=confirmation_edge_shift_offset
    )
    confirmation = _matrix_projection("confirmation", selected=structural["selected"])
    return {
        "terminal_state": "pass",
...
def _terminal_base() -> dict[str, object]:
    observation = P4._expected_python_process_observation(ROOT, mode="--run")
    return {
        ...
        "python_process_pre_import_observation": observation,
        "python_process_post_import_observation": observation,
        "python_process_post_execution_observation": observation,
        "source_closure_pre_sha256": "6" * 64,
        "source_closure_post_sha256": "6" * 64,
...
def _caught_external_bundle(path: Path) -> tuple[dict[str, object], dict[str, bytes]]:
    ...
    path.mkdir()
    calibration_selector = _selector_projection()
    ...
    attempt = {
        "schema_version": P4.ATTEMPT_SCHEMA_VERSION,
        "experiment_id": P4.EXPERIMENT_ID,
    ...
    graph = {
        "schema_version": P4.GRAPH_SELECTION_SEAL_SCHEMA_VERSION,
        "experiment_id": P4.EXPERIMENT_ID,
    ...
    threshold = {
        "schema_version": P4.THRESHOLD_SEAL_SCHEMA_VERSION,
        "experiment_id": P4.EXPERIMENT_ID,
    ...
    confirmation = {
        "schema_version": P4.CONFIRMATION_ACCESS_SEAL_SCHEMA_VERSION,
        "experiment_id": P4.EXPERIMENT_ID,
        "attempt_sha256": attempt_sha256,
        "launch_sha256": launch_sha256,
        "graph_selection_seal_sha256": graph_sha256,
...
def _synthetic_confirmation_assessment() -> tuple[dict[str, object], dict[str, object]]:
    side = 7
    ...
    graph_input = P4.GraphInput(
        primary_unit_id="synthetic-v0-2-confirmation-round-trip",
        vertex_ids=np.arange(side * side, dtype="<i8"),
        states=states,
    )
    selector = _selector_projection()
    structural, _triplet = P4.assess_fixed_triplet(
        graph_input,
        _oriented_grid_faces(side),
        {"cycle_classes": {"central": [2, 2, 4, 4], "wide": [1, 1, 5, 5]}},
        selector["selected"],
    )
    return selector, structural


def test_selector_and_confirmation_measurement_keysets_are_exact_15_and_13() -> None:
    selector = _selector_projection()
    confirmation = _confirmation_structural(selector)

    assert set(selector["triplet_measurements"]) == SELECTOR_MEASUREMENT_KEYS
    assert set(confirmation["triplet_measurements"]) == (CONFIRMATION_MEASUREMENT_KEYS)
    assert len(SELECTOR_MEASUREMENT_KEYS) == 15
    assert len(CONFIRMATION_MEASUREMENT_KEYS) == 13
    assert set(P4.SELECTOR_TRIPLET_MEASUREMENT_KEYS) == SELECTOR_MEASUREMENT_KEYS
    assert set(P4.CONFIRMATION_TRIPLET_MEASUREMENT_KEYS) == (
        CONFIRMATION_MEASUREMENT_KEYS
    )
...
def test_selector_producer_canonical_round_trip_emits_exact_15_keys() -> None:
    ...
    selector = P4.select_structural_triplet(
        graph_input,
        _oriented_grid_faces(side),
        {"cycle_classes": {"central": [2, 2, 4, 4], "wide": [1, 1, 5, 5]}},
    )
    persisted = P4.parse_canonical_json(
        P4.canonical_json_bytes(selector),
        label="selector projection round trip",
    )

    assert persisted["state"] == "pass"
    assert set(persisted["triplet_measurements"]) == SELECTOR_MEASUREMENT_KEYS
    assert P4._validate_selector_projection(persisted) == persisted


def test_confirmation_producer_canonical_round_trip_emits_exact_13_keys() -> None:
    selector, structural = _synthetic_confirmation_assessment()
    persisted = P4.parse_canonical_json(
        P4.canonical_json_bytes(structural),
        label="confirmation structural round trip",
    )

    assert set(persisted["triplet_measurements"]) == CONFIRMATION_MEASUREMENT_KEYS
    assert (
        P4._validate_confirmation_structural_projection(
            persisted,
            selector=selector,
            expected_state=persisted["state"],
        )
        == persisted
    )


def test_confirmation_structural_insufficient_round_trips_exact_13_keys() -> None:
    selector = _selector_projection()
    structural = _confirmation_structural(selector, passing=False)
    persisted = P4.parse_canonical_json(
        P4.canonical_json_bytes(structural),
        label="insufficient confirmation structural round trip",
    )
...
def test_confirmation_selector_rerun_must_remain_false() -> None:
    selector = _selector_projection()
    structural = _confirmation_structural(selector)
    structural["selector_rerun"] = True

    with pytest.raises(P4.P4ProtocolError, match="confirmation selector rerun"):
        P4._validate_confirmation_structural_projection(
            structural,
            selector=selector,
            expected_state="pass",
...
@pytest.mark.parametrize("key", sorted(SELECTOR_ONLY_MEASUREMENT_KEYS))
def test_confirmation_measurements_reject_each_selector_only_key(key: str) -> None:
    selector = _selector_projection()
    structural = _confirmation_structural(selector)
    structural["triplet_measurements"][key] = (
        False if key == "jaccard_used_as_objective" else []
    )

    with pytest.raises(P4.P4ProtocolError, match="keys differ"):
        P4._validate_confirmation_structural_projection(
            structural,
            selector=selector,
            expected_state="pass",
...
@pytest.mark.parametrize("key", sorted(SELECTOR_ONLY_MEASUREMENT_KEYS))
def test_selector_measurements_require_each_selector_only_key(key: str) -> None:
    selector = _selector_projection()
    del selector["triplet_measurements"][key]

    with pytest.raises(P4.P4ProtocolError, match="keys differ"):
        P4._validate_selector_projection(selector)


def test_selector_and_confirmation_measurement_payloads_are_not_interchangeable() -> (
    None
):
    selector = _selector_projection()
    confirmation = _confirmation_structural(selector)
    ...
    with pytest.raises(P4.P4ProtocolError, match="keys differ"):
        P4._validate_selector_triplet_measurements(
            confirmation["triplet_measurements"],
            selected=confirmation["selected"],
        )
    with pytest.raises(P4.P4ProtocolError, match="keys differ"):
        P4._validate_confirmation_triplet_measurements(
            selector["triplet_measurements"],
            selected=selector["selected"],
        )


@pytest.mark.parametrize("key", sorted(CONFIRMATION_MEASUREMENT_KEYS))
def test_base_measurements_reject_each_missing_key_and_unknown_key(key: str) -> None:
    selector = _selector_projection()
    confirmation = _confirmation_structural(selector)
    del confirmation["triplet_measurements"][key]
    ...
    unknown = _confirmation_structural(selector)
    unknown["triplet_measurements"]["unknown"] = None
    with pytest.raises(P4.P4ProtocolError, match="keys differ"):
        P4._validate_confirmation_structural_projection(
            unknown,
            selector=selector,
...
def test_v02_protocol_freezes_predecessor_and_unconstructed_new_input(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def forbidden(*_args: object, **_kwargs: object) -> object:
        raise AssertionError("source-only protocol validation constructed an input")

    monkeypatch.setattr(P4.CartesianFourierDomainGenerator, "generate", forbidden)
    monkeypatch.setattr(P4, "_recompute_calibration_selector", forbidden)

    protocol = P4.load_and_validate_protocol(ROOT)

    assert protocol["status"] == "frozen_not_run"
    assert protocol["predecessor_binding"] == P4._predecessor_binding_document()
    assert protocol["confirmation_input_binding"] == (
        P4._confirmation_input_binding_document()
    )
    assert protocol["measurement_schemas"] == P4._measurement_schemas_document()
    assert protocol["substrates"]["calibration"]["seed"] == 314159
    assert protocol["substrates"]["confirmation"] == {
    ...
    assert (
        protocol["confirmation_input_binding"]["generated_input_disjointness_observed"]
        is False
    )
    assert protocol["confirmation_input_binding"]["independence_restored"] is False
    assert (
        protocol["successor_authorization"]["attempt_authorized_by_source_freeze"]
        is False
    )


def test_v02_predecessor_binding_rejects_semantic_or_digest_tamper() -> None:
    semantic = P4._predecessor_binding_document()
    semantic["authority_transfer"] = True
    with pytest.raises(P4.P4ProtocolError, match="predecessor binding"):
        P4._validate_predecessor_evidence(ROOT, semantic)

    digest = P4._predecessor_binding_document()
    digest["terminal"]["sha256"] = "0" * 64
    with pytest.raises(P4.P4ProtocolError, match="predecessor binding"):
        P4._validate_predecessor_evidence(ROOT, digest)


def test_v02_namespace_is_disjoint_from_consumed_v01_namespace() -> None:
    assert P4.EXPERIMENT_ID != P4.PREDECESSOR_EXPERIMENT_ID
    assert P4.REPOSITORY_PROTOCOL != P4.PREDECESSOR_PROTOCOL
    assert P4.REPOSITORY_RUNNER != P4.PREDECESSOR_RUNNER
    assert P4.REPOSITORY_LAUNCH != P4.PREDECESSOR_LAUNCH
    assert P4.REPOSITORY_ATTEMPT != P4.PREDECESSOR_ATTEMPT
    assert P4.REPOSITORY_TERMINAL != P4.PREDECESSOR_TERMINAL
    assert "v0-2" in str(P4.EXTERNAL_STAGE)
    assert "v0-2" in str(P4.EXTERNAL_STORE)
    assert "v0-2" in str(P4._BOOTSTRAP_PYCACHE_PREFIX)
    assert P4.REPOSITORY_RUNNER == Path(
        "scripts/run_p4_graph_evaluability_calibration_v0_2.py"
    )
...
def test_no_eligible_triplet_has_no_winner_or_objective() -> None:
    ...
    only_one_family = (_candidate("mutual-knn", (("neighbor_count", 6),), edges, "a"),)
...
def test_frozen_selector_recomputation_rejects_refreshed_alternate_winner(
    ...
    actual = _selector_projection()
    monkeypatch.setattr(
        P4,
        "_recompute_calibration_selector",
        lambda _protocol: types.SimpleNamespace(projection=actual),
    )
...
def test_selector_radius_scan_audits_budget_out_zero_pairs(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    graph_input = P4.GraphInput(
    ...
    assert (
        result["selector_audit"]["radius_budget_eligible_zero_unrepresentable"] is False
    )
...
def test_nonnegative_finite_scalar_is_fail_closed(
...
def test_matrix_rejects_axis_and_status_value_forgery() -> None:
    ...
    status_forge["cells"][0]["reason_codes"] = ["boundary_amplitude_at_or_below_floor"]
...
def test_confirmation_matrix_binds_held_out_structural_edges_not_calibration() -> None:
    ...
    calibration_by_family = {
        item["family"]: item["edge_fingerprint_sha256"] for item in selector_selected
    }
...
def test_complete_terminal_builder_accepts_exact_13_confirmation_payload() -> None:
    result = _full_attempted_result()
    terminal = P4._build_terminal(
        base=_terminal_base(),
        attempt_sha256="7" * 64,
    ...
    persisted = P4.parse_canonical_json(
        P4.canonical_json_bytes(terminal),
        label="complete terminal round trip",
    )

    assert persisted == terminal
    assert (
        set(persisted["result"]["confirmation_structural"]["triplet_measurements"])
        == CONFIRMATION_MEASUREMENT_KEYS
    )
    assert persisted["execution_terminal"] == "complete"
...
def test_self_consistent_pass_labels_cannot_forge_numeric_control() -> None:
    ...
    row = next(
        item
        for item in forged["controls"]
        if item["control_id"] == "field_only_shuffle"
    )
...
def test_graph_and_threshold_seals_cross_bind_semantic_result(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    result = _full_attempted_result()
    protocol = P4._load_canonical(
    ...
    exact_selector = _selector_projection()
    graph_result["calibration_selector"] = exact_selector
    monkeypatch.setattr(
        P4,
        "_recompute_calibration_selector",
        lambda _protocol: types.SimpleNamespace(projection=exact_selector),
    )
    attempt_sha256 = "4" * 64
    launch_sha256 = "5" * 64
    graph_sha256 = "6" * 64
    ...
    graph_forge["calibration_selector"]["selected"][0]["graph_fingerprint_sha256"] = (
        "7" * 64
    )
    with pytest.raises(P4.P4ProtocolError, match="frozen recomputation"):
        P4._validate_graph_selection_seal(
...
def test_exact_argv_isolated_from_existing_project_bytecode() -> None:
    ...
    assert argv[-2] == str(ROOT / P4.REPOSITORY_RUNNER)
    assert "v0-2" in prefix
    assert "v0-1" not in prefix


def test_expected_run_and_prepare_process_flags_are_exact() -> None:
    run = P4._expected_python_process_observation(ROOT, mode="--run")
    prepare = P4._expected_python_process_observation(ROOT, mode="--prepare-launch")
    ...
    assert run["base_executable"] == str(P4._BOOTSTRAP_RESOLVED_BASE_EXECUTABLE)
    ...
    assert run["physical_launcher_sha256"] == (P4._BOOTSTRAP_PHYSICAL_LAUNCHER_SHA256)
...
def test_live_macos_framework_python_coordinates_match_source_freeze() -> None:
    ...
    assert (
        P4._bootstrap_stable_regular_sha256(
            P4._BOOTSTRAP_RESOLVED_BASE_EXECUTABLE,
...
def test_runtime_binding_seals_logical_base_and_physical_launcher(
    ...
    assert runtime["python_logical_executable"] == str(P4._BOOTSTRAP_LOGICAL_EXECUTABLE)
...
def test_loaded_module_origin_is_bound_regular_nlink_one_source(
    ...
    assert P4._loaded_spirallens_source_manifest(repo, protocol, modules=modules) == [
        {
...
runner = root / "scripts" / "run_p4_graph_evaluability_calibration_v0_2.py"
...
def test_control_verdict_distinguishes_unresolved_from_wrong() -> None:
    ...
    state, reason = P4._fold_attempted_controls(controls, oracle_ok=True, span_ok=True)
...
def test_v02_unit_import_preserves_lifecycle_state_and_v01_evidence(
    tmp_path: Path,
) -> None:
    script = r"""
import hashlib
...
"""
    ...
    assert P4._sha256_file(ROOT / P4.PREDECESSOR_LAUNCH) == (
        P4.PREDECESSOR_LAUNCH_SHA256
    )
    assert P4._sha256_file(ROOT / P4.PREDECESSOR_ATTEMPT) == (
        P4.PREDECESSOR_ATTEMPT_SHA256
    )
    assert P4._sha256_file(ROOT / P4.PREDECESSOR_TERMINAL) == (
        P4.PREDECESSOR_TERMINAL_SHA256
    )
    launch = json.loads((ROOT / P4.PREDECESSOR_LAUNCH).read_bytes())
    attempt = json.loads((ROOT / P4.PREDECESSOR_ATTEMPT).read_bytes())
    terminal = json.loads((ROOT / P4.PREDECESSOR_TERMINAL).read_bytes())
    assert (
        launch["source_commit"] == attempt["source_commit"] == terminal["source_commit"]
    )
...
def test_external_manifest_validates_exact_members_and_three_seal_chain(
    ...
    _install_synthetic_selector_recompute(monkeypatch)
    store = tmp_path / "store"
    launch, sources = _caught_external_bundle(store)

    observed = P4._validate_external_bundle_path(store, repo_root=ROOT, launch=launch)

    assert (
        observed["attempt_sha256"]
        == hashlib.sha256(sources[P4.ATTEMPT_NAME]).hexdigest()
    )
    assert (
        observed["terminal_sha256"]
        == hashlib.sha256(sources[P4.TERMINAL_NAME]).hexdigest()
    )
    assert (
        observed["manifest_sha256"]
        == hashlib.sha256(sources[P4.STORE_MANIFEST_NAME]).hexdigest()
    )
...
def test_caught_error_bundle_rejects_fully_rehashed_alternate_selector(
    ...
    (store / P4.STORE_MANIFEST_NAME).write_bytes(P4.canonical_json_bytes(manifest))
...
def test_external_manifest_rejects_extra_member_and_chain_rewrite(
    ...
    with pytest.raises(P4.P4ProtocolError, match="member set"):
        P4._validate_external_bundle_path(extra_store, repo_root=ROOT, launch=launch)
    ...
    terminal = P4._load_canonical(chain_store / P4.TERMINAL_NAME, label="test terminal")
    ...
    with pytest.raises(P4.P4ProtocolError, match="graph-selection chain"):
        P4._validate_external_bundle_path(chain_store, repo_root=ROOT, launch=launch)
...
def test_atomic_projection_promotion_failure_leaves_repairable_exact_temp(
    ...
    monkeypatch.setattr(
        P4,
        "_native_file_no_replace",
        lambda *_args: os.errno.EIO if hasattr(os, "errno") else 5,
    )

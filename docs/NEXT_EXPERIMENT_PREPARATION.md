...
The PR27 source commit defines the remaining item-21 positive-authority
...
PR27 documentation. The receipt is the only addition in its direct-child
...
completing item 21 at the final PR27 tip.

Historical reload remains tied to the PR27 source-commit tree and runtime-lock
blob. The current-readiness check separately rejects later live source or
runtime drift. Since item-22 orchestration is not introduced by PR27, any
...
verifies only the declared historical Git source set; the final PR27 chain
...
Their authority and verification fields remain false; the PR27 item-21
...
3. retain item 21's exact `requirements-d7-runtime-lock.txt`, fixed
   ...
   ingredients only; after all PR27 source is final, add only the exact
...
Nothing in the PR27 source commit or its three artifact-only children performs
item 22. Even at the final PR27 tip, no exclusive supplier claim or invocation,

...
| `spirallens.contracts` | provisional; bounded source/direct-wheel namespace observation only | the ordered-export manifest owns the exact seven root names. The SHA-fixed focused test runs exact 42 source nodes and exact 41 wheel-safe nodes—the source manifest/consumer join alone is excluded—at the exact CPython 3.11.16, 3.12.14, and 3.13.15 / Ubuntu 24.04 x86_64 locked-dependency direct-wheel coordinates. It binds the root/defining identities, exact installed origins, enumerated behavior/failures, and the exact source-static direct-root import graph; module membership alone is not a consumer-independence decision. Per-name review records `established` only for `ContinuousHolonomy` and `SampledLoop`, and `not_established` for `CalibrationCheck`, `CalibrationReport`, `LoopOrientation`, `SampledWinding`, and `WindingEstimate`. One provisional source line makes default-generated `SampledLoop.parameter_values` read-only like explicit inputs, with no numeric value, signature, export, or dependency delta. This is not runtime or installed consumer evidence, full library-test ownership, coverage of the other 552 exports or sdist-derived behavior, namespace admission, or promotion: `closed_library_allowlist_established=false`; `closed_public_api_contract_established=false`; all distribution grants remain false; support, stability, compatibility, portability, typing, and release remain unestablished; `LIB-L0` remains in progress; no science, authority, or D7 state changes; historical source/D7 receipts stay unchanged and unre-anchored, and current readiness remains false |
| `spirallens.calibration`, `spirallens.loops`, `spirallens.holonomy`, `spirallens.topology` | provisional; bounded analytic source/direct-wheel observation only | the exact three SHA-fixed phantom files run all exact 25 nodes in source and a neutral direct wheel at the exact CPython 3.11.16 / 3.12.14 / 3.13.15, Ubuntu 24.04 x86_64, locked-dependency coordinates, with exact 15-module origin closure and exact 24-check ordered name/category, finiteness, and pass recomputation. This is a model-free vertical analytic slice only: it establishes no namespace admission or promotion, second independent consumer, full exact-27 coverage (`affine_transform_loop` and `signed_area_2d` remain unobserved), sdist behavior, repository independence, portability, support, API, compatibility, release, schema, science, authority, D7 state, or `LIB-L0` completion |
| `spirallens.synthetic`, `spirallens.graphs` | provisional; bounded graph source/direct-wheel observation only | three SHA-fixed full files run exact 27 graph nodes (6 diversity + 10 discrete-domain + 11 cycle-binding) in source and a neutral direct wheel at the exact three locked CPython / Ubuntu coordinates. The probe binds exact 41 graph-root exports, exact seven operation identities, exact nine source/installed origins, zero attempts across exact nine blocked optional/repository prefixes, and the package-internal `constructors.py` source-byte digest recorded by construction. These operations have one qualification vertical consumer, not two independent consumers. This is not full graph/library-test ownership, persistence round-trip, graph equivalence, replication, homology, winding, namespace admission/promotion, sdist behavior, portability, compatibility, API/support/stability/typing/release, science/authority/D7 change, or `LIB-L0` completion; synthetic scientific qualification remains artifact-bound |

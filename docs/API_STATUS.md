...
| `spirallens.atlas` | provisional/model extra | manifest-reader imports retain NumPy/PyYAML but load no model framework, adapter, or capture runtime. The five `id_sweep` module/root bindings resolve with model prefixes blocked; neutral class/function hints resolve, while `run_id_sweep` remains model-extra and fails at its first execution statement on missing Torch before reading adapter/config. Its raw signature and annotation spellings remain fixed; default runtime `get_type_hints(run_id_sweep)` is outside this boundary. Star import still reaches the blocked engineering runner, so this does not make the whole namespace framework-neutral or promote an API. The repository-bound public-example runner still requires an explicit matching `repository_root` |

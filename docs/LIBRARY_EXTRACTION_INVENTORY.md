...
| exact-27 graph diversity/domain/cycle-binding slice | provisional; bounded model-free source/direct-wheel observation only | present; NumPy; exact three SHA-fixed full files run 6 + 10 + 11 nodes in source and a neutral direct wheel at the exact CPython 3.11.16 / 3.12.14 / 3.13.15, Ubuntu 24.04 x86_64, locked-dependency coordinates | in-memory graph receipts only; construction intentionally reads and digests package-internal `spirallens/graphs/constructors.py` source bytes; no persistence, science, or authority meaning | one qualification vertical lane, `spirallens.qualification.crossed`, consumes exact seven operations: `bind_cycle_class`, `build_discrete_domain_complex`, `construct_mutual_knn`, `construct_radius_graph`, `construct_shared_neighbor_graph`, `define_boundary_cycle_class`, `measure_graph_diversity` | no second independent production consumer established | exact 41 root exports, exact seven defining identities, exact nine loaded/origin closure, and zero attempts across exact nine blocked optional/repository prefixes | only the selected exact 27 behavior/failures are observed | one test-file delta and workflow plus docs only; no production, export, dependency, manifest, validator, report, or schema delta | no full graph/library-test ownership, persistence round-trip, graph equivalence, replication, homology, winding, namespace admission/promotion, sdist behavior, portability, compatibility, support/API/stability/typing/release, science/authority/D7 change, or `LIB-L0` completion |

#!/usr/bin/env python3
"""Frozen, model-free P4 graph evaluability calibration.
...
No integer or topology claim is emitted.
...
_BOOTSTRAP_GUARDED_MODE = next(
    (
        mode
        for mode in ("--run", "--prepare-launch")
...
from spirallens.graphs.common import array_sha256, coordinate_order_invariant_euclidean_norm
...
SCHEMA_VERSION = "spirallens.p4-graph-evaluability-terminal.v0.1"
PROTOCOL_SCHEMA_VERSION = "spirallens.p4-graph-evaluability-protocol.v0.1"
LAUNCH_SCHEMA_VERSION = "spirallens.p4-graph-evaluability-launch.v0.1"
EXPERIMENT_ID = "p4-graph-evaluability-calibration-v0.1"

REPOSITORY_PROTOCOL = Path("protocols/p4_graph_evaluability_calibration_v0_1.json")
REPOSITORY_RUNNER = Path("scripts/run_p4_graph_evaluability_calibration.py")
...
EXTERNAL_STAGE = Path(
    ...
    ".spirallens-p4-graph-evaluability-calibration-v0-1-store.staging"
)
EXTERNAL_STORE = Path(
    "/Users/ryohiga/SpiralReality/"
    "spirallens-p4-graph-evaluability-calibration-v0-1-store"
)
...
def _cell_id(
    ...
    return "|".join(
        (role, case_name, cycle_class, field_family, cycle_family)
    )
...
class P4ProtocolError(ValueError):
    """Raised before the official attempt is consumed."""
...
def load_and_validate_protocol(
    ...
    _constant(protocol["decision_date"], "2026-08-24", label="decision_date")
    ...
    next_experiment = _mapping(hypotheses.get("next_experiment"), label="next_experiment")
    ...
    _constant(confirmation["seed"], 271828, label="confirmation.seed")
    _constant(confirmation["density_warp_strength"], 0.25, label="confirmation.warp")
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "specification_fingerprint_sha256": (
                self.specification_fingerprint_sha256
            ),
...
def choose_graph_triplet(
    ...
    audit: dict[str, object] = {
        ...
        "all_per_graph_decisions_sha256": canonical_json_sha256(
            per_graph_decisions
        ),
...
def _candidate_from_receipt(
    ...
    *,
    ...
    return StructuralCandidate(
        ...
        two_core_rows=frozenset(int(row) for row in np.flatnonzero(receipt.two_core_mask)),
...
def _cycle_context(
    ...
    domain = build_discrete_domain_complex(
        graph_input,
        oriented_faces,
        domain_id=f"p4-domain-{graph_input.primary_unit_id}",
    ...
    for name in ("central", "wide"):
        ...
        classes[name] = define_boundary_cycle_class(
            ...
            cycle_class_spec_id=f"p4-{name}",
            primary_unit_id=graph_input.primary_unit_id,
            matched_set_id=f"p4-matched-{name}",
        )
    rule = BoundaryRefinementRule(
        rule_id="p4-forward-span-four",
...
def enumerate_structural_candidates(
    ...
    for neighbor_count in range(2, 17):
        receipt = construct_mutual_knn(
            graph_input,
            MutualKnnSpec(
                spec_id=f"p4-mutual-k{neighbor_count:02d}",
    ...
    for rank, radius in enumerate(unique_distances):
        ...
        if radius == 0.0:
            continue
        radius_digest = hashlib.sha256(float(radius).hex().encode("ascii")).hexdigest()[:12]
        receipt = construct_radius_graph(
            graph_input,
            RadiusGraphSpec(
                spec_id=f"p4-radius-r{rank:04d}-{radius_digest}",
                purpose=GraphPurpose.CYCLE_CONSTRUCTION,
                radius=radius,
            ),
        )
    ...
    for neighbor_count in range(2, 17):
        for minimum_shared_neighbors in range(1, neighbor_count + 1):
            receipt = construct_shared_neighbor_graph(
                ...
                SharedNeighborSpec(
                    spec_id=(
                        f"p4-shared-k{neighbor_count:02d}-"
...
def select_structural_triplet(
    ...
    zero_unrepresentable = bool(
        counts["radius_budget_eligible_zero_unrepresentable"]
    )
...
def _construct_specified(
    ...
    *,
    ...
    spec = _spec_from_document(
        family, parameters, purpose=purpose, graph_id=graph_id
    )
...
def _graph_axes(selected: Sequence[object]) -> GraphAxes:
    ...
    for prefix, purpose, target in (
        ...
        for raw in selected:
            ...
            target.append(
                GraphDeclaration(
                    graph_id=f"p4-{prefix}-{family.value}",
                    family=family,
                    purpose=purpose,
                    parameters=tuple(
...
def _loop_policy() -> LoopPhasePolicy:
    return LoopPhasePolicy(
        policy_id="p4-model-free-continuous-readout",
...
def _matrix_readouts(
    *,
    ...
    for case_name, expected in CROSSED_CASES:
        ...
        graph_input = GraphInput(
            primary_unit_id=f"p4-{role}-{case_name.replace('_', '-')}",
        ...
        for class_name, rectangle_raw in _mapping(
            ...
            execution = build_crossed_graph_execution(
                ...
                domain_id=f"p4-{role}-{case_name}-{class_name}",
                cycle_class_spec_id=f"p4-{class_name}",
                matched_set_id=f"p4-{role}-{class_name}",
                refinement_rule=BoundaryRefinementRule(
                    rule_id="p4-forward-span-four",
                    max_domain_edges_per_graph_edge=4,
                ),
            ...
            for estimate, field_graph in zip(estimates, execution.field_graphs, strict=True):
        ...
        spans.append(
            {
                ...
                "case": case_name,
                "cycle_class": class_name,
                "graph_family_span_cycles": max(totals) - min(totals) if totals else None,
            }
        )
    return (
        {
...
def _edge_content_sha256(receipt: GraphConstructionReceipt) -> str:
    return canonical_json_sha256(
        [list(edge) for edge in sorted(_edge_ids(receipt))]
    )
...
def _graph_invariance_controls(
    ...
    for index, raw in enumerate(selected_documents):
        ...
        base.append(
            _construct_specified(
                ...
                purpose=GraphPurpose.CYCLE_CONSTRUCTION,
                graph_id=f"p4-control-base-{index}",
            )
        )

    permutation = np.arange(48, -1, -1, dtype="<i8")
    permuted_input = GraphInput(
        primary_unit_id="p4-joint-permutation",
    ...
    orthogonal_input = GraphInput(
        primary_unit_id="p4-ambient-orthogonal",
        vertex_ids=graph_input.vertex_ids,
        states=orthogonal_states,
    )
    scaled_input = GraphInput(
        primary_unit_id="p4-global-scaling",
        vertex_ids=graph_input.vertex_ids,
        states=2.0 * graph_input.states,
    )
    for index, (raw, base_graph) in enumerate(zip(selected_documents, base, strict=True)):
        item = _mapping(raw, label="selected graph")
        ...
        permuted_graph = _construct_specified(
            ...
            parameters,
            purpose=GraphPurpose.CYCLE_CONSTRUCTION,
            graph_id=f"p4-control-permuted-{index}",
        )
        orthogonal_graph = _construct_specified(
            ...
            parameters,
            purpose=GraphPurpose.CYCLE_CONSTRUCTION,
            graph_id=f"p4-control-orthogonal-{index}",
        )
        ...
        scaled_graph = _construct_specified(
            ...
            scaled_parameters,
            purpose=GraphPurpose.CYCLE_CONSTRUCTION,
            graph_id=f"p4-control-scaled-{index}",
        )
        ...
        scaling_equal = (
            base_graph.edge_order_sha256 == scaled_graph.edge_order_sha256
        )
        ...
        transformed_vertex_ids = [
            int(value) for value in permuted_input.vertex_ids
        ]
        ...
        common_evidence = {
            ...
            "sealed_family_edge_order_sha256": item[
        ...
        joint_observations.append(
            {
                ...
                **common_evidence,
                ...
                "base_vertex_order_sha256": array_sha256(
                ...
                "transformed_edge_content_sha256": _edge_content_sha256(
                    permuted_graph
                ),
                "transformed_edge_order_sha256": (
                    permuted_graph.edge_order_sha256
                ),
            }
        )
        ambient_observations.append(
            {
                ...
                **common_evidence,
                ...
                "transformed_edge_order_sha256": (
                    orthogonal_graph.edge_order_sha256
                ),
            }
        )
...
def _collapsed_cycleless_control(
    ...
    collapsed_input = GraphInput(
        primary_unit_id="p4-collapsed-rank-one-path",
        vertex_ids=graph_input.vertex_ids,
        states=states,
    )
    receipt = construct_radius_graph(
        collapsed_input,
        RadiusGraphSpec(
            spec_id="p4-collapsed-unit-radius-path",
            purpose=GraphPurpose.CYCLE_CONSTRUCTION,
            radius=1.0,
        ),
    )
...
def _derived_blind(base: object, *, control_id: str, section: np.ndarray, coherence: np.ndarray) -> object:
...
def _control_receipts(
    *,
    ...
    del calibration_matrix  # Calibration values set thresholds; controls use held-out cells.
    contracts = {
    ...
    for cycle_class in CYCLE_CLASS_ORDER:
        ...
        if np.any(amplitudes <= 0.0):
            ...
            transform_nonidentity = not np.array_equal(
                shifted_amplitudes, amplitudes
            )
    ...
    density_cell_values = [confirmation_cells.get(item) for item in density_required_cells]
    density_span_values = [confirmation_spans.get(item) for item in density_required_spans]
    density_cells_evaluable = len(density_required_cells) == 54 and all(
        item is not None
        and item.get("attempt_status") == "evaluable"
    ...
    controls.extend(_graph_invariance_controls(calibration_graph_input, selected_documents))
    controls.append(
    ...
    controls.append(
        {
            ...
            "observed_state": (
                ...
                and all(
                    item["state"] == "pass" for item in field_shuffle_observations
                )
                else "fail"
            ),
    ...
    for control_id in expected_ids:
        ...
        observations = _sequence(item.get("observations"), label=f"{control_id} observations")
        observed_ids = [
...
def _recompute_calibration_selector(
    ...
    calibration_phantom = CartesianFourierDomainGenerator().generate(
        calibration_spec
    )
    calibration_inputs = calibration_phantom.positive.estimator_inputs
    calibration_graph_input = GraphInput(
        primary_unit_id="p4-calibration-selector",
...
def execute_calibration(
    ...
    *,
    ...
    calibration_cells = _sequence(calibration_matrix["cells"], label="calibration cells")
    raw_errors = [
    ...
    converted_algebraic = [
        _nonnegative_finite_scalar(value) for value in raw_algebraic
    ]
    ...
    scalar_inventory = {
        ...
        "pure_so2_gauge_error_cycles": (
            1 if converted_algebraic[0] is not None else 0
        ),
        "orientation_reversal_error_cycles": (
    ...
    algebraic_resolved = all(
        ...
        and float(_mapping(value, label="algebraic diagnostic")["error_cycles"])
        <= 1e-8
        for value in algebraic.values()
    )
    ...
    confirmation_graph_input = GraphInput(
        primary_unit_id="p4-confirmation-fixed-triplet",
    ...
    confirmation_cells = _sequence(confirmation_matrix["cells"], label="confirmation cells")
    oracle_ok = all(
        (value := _nonnegative_finite_scalar(item["absolute_error_cycles"]))
        is not None
        and value <= oracle_threshold
        for item in (_mapping(raw, label="confirmation cell") for raw in confirmation_cells)
    )
    span_ok = all(
        ...
        for item in (
            ...
            for raw in _sequence(confirmation_matrix["graph_family_spans"], label="spans")
        )
    )
...
def _require_clean_except_projections(repo_root: Path) -> None:
    ...
    unexpected = [
        line for line in _split_nul(output, label="Git projection status")
...
def _source_paths(protocol: Mapping[str, object]) -> tuple[Path, ...]:
    ...
    paths = [Path(str(runner["path"])), REPOSITORY_PROTOCOL, REPOSITORY_HYPOTHESES]
    ...
    *,
    ...
    for module_name in sorted(namespace):
        ...
        if (
            ...
            or (origin_metadata.st_dev, origin_metadata.st_ino)
        ...
        records.append(
            {"module": module_name, "path": relative, "sha256": digest}
        )
...
def _read_projection_candidate(path: Path, *, label: str) -> tuple[bytes, os.stat_result]:
    try:
...
def _publish_repository_projection(path: Path, source: bytes) -> str:
    ...
    try:
        ...
        raise P4ProtocolError("repository projection source is not canonical") from error
    parent = path.parent
    ...
    try:
        ...
        if not _entry_absent(path):
            ...
            if not _entry_absent(temporary):
                ...
                if not source.startswith(staged) or staged_meta.st_uid != os.geteuid():
                    raise P4ProtocolError("stale repository projection temp is not owned prefix")
        ...
        if _entry_absent(temporary):
            ...
            if staged_meta.st_uid != os.geteuid() or not source.startswith(staged):
                raise P4ProtocolError("repository projection temp is not an owned exact prefix")
            mode = stat.S_IMODE(staged_meta.st_mode)
            if len(staged) < len(source) and mode != 0o600:
                raise P4ProtocolError("partial repository projection temp is not writable-owned")
            descriptor = os.open(
                temporary.name,
        ...
        try:
            ...
            if prefix_size > len(source):
                raise P4ProtocolError("repository projection temp exceeds intended bytes")
            if prefix_size < len(source):
                ...
                while view:
                    written = os.write(descriptor, view)
                    if written <= 0:
                        raise P4PersistenceError("repository projection temp write stalled")
                    view = view[written:]
                os.fsync(descriptor)
        ...
        os.fsync(parent_fd)
        observed_errno = _native_file_no_replace(
            parent_fd, temporary.name, path.name
        )
        ...
        if final_source != source:
            raise P4PersistenceError("repository projection target differs after publish")
        return "published"
...
def _validate_stage_anchor(stage: _OwnedStage) -> None:
    try:
        ...
        if (held.st_dev, held.st_ino) != (stage.device, stage.inode):
            raise P4PersistenceError(
                "held external stage descriptor identity changed"
            )
        ...
        if (live.st_dev, live.st_ino) != (stage.device, stage.inode):
            raise P4PersistenceError(
                "external stage/store namespace identity changed"
            )
    except OSError as error:
...
def _promote_external_stage(stage: _OwnedStage) -> None:
    ...
    if function is None:
        raise P4PersistenceError(
            "native no-replace directory promotion disappeared"
        )
    function.argtypes = (
        ctypes.c_int,
        ctypes.c_char_p,
...
def _reconstruct_canonical_graph(
    ...
    *,
    ...
    if canonical != sorted(set(canonical)):
        raise P4RunError(
            f"{label} canonical edges must be sorted and unique"
        )
    ...
    return {
        ...
        "two_core_rows": frozenset(
            int(row) for row in np.flatnonzero(two_core)
        ),
        "edge_count": edge_count,
        "component_count": component_count,
...
def _validate_structural_candidate_projection(
    ...
    *,
    ...
    if family == GraphFamily.MUTUAL_KNN.value:
        ...
        radius = _require_finite_scalar(
            parameters["radius"], label=f"{label} radius"
        )
    ...
    reconstructed = _reconstruct_canonical_graph(
        item["canonical_edges"], label=label
    )
    for key in (
        "edge_count",
        "component_count",
    ...
    _constant(item["mean_degree"], 2.0 * edge_count / 49.0, label=f"{label} mean degree")
...
def _validate_triplet_measurements(
    value: object,
    *,
    ...
    for key, expected in derived.items():
        _constant(measurements[key], expected, label=f"triplet {key}")
    common_core = len(
        set.intersection(
    ...
    _constant(
        measurements["common_two_core_intersection_count"],
        common_core,
    ...
    pairwise = [
        ...
        for item in _sequence(measurements["pairwise"], label="selector pairwise")
    ]
    ...
    all_different = True
    all_within = True
    for item, (left_index, right_index) in zip(
        pairwise, expected_pairs, strict=True
    ):
        _exact_keys(
            item,
            {
    ...
    _constant(
        measurements["pairwise_edge_sets_must_differ"],
        all_different,
        label="triplet pairwise distinct fold",
    )
    _constant(
        measurements["pairwise_edge_jaccard_at_most_0_85"],
        all_within,
    ...
    if require_gates:
        ...
        if derived["largest_component_vertex_count_spread"] > 2 or derived[
            "two_core_vertex_count_spread"
        ] > 4:
...
def _validate_selector_projection(value: object) -> dict[str, object]:
    ...
    _constant(
        ...
        "spirallens.p4-selector-projection.v0.1",
    ...
    _constant(
        audit["radius_float64_uint64_order"], True, label="radius bit ordering"
    )
    ...
    if state == "pass":
        ...
        measurements = _validate_triplet_measurements(
        ...
        if selector["triplet_measurements"] is not None:
            raise P4RunError("insufficient selector cannot persist triplet measurements")
...
def _validate_confirmation_structural_projection(
    ...
    *,
    ...
    _exact_keys(
        structural,
        {"state", "reason", "selected", "triplet_measurements", "selector_rerun"},
        label="confirmation structural",
    )
    _constant(structural["state"], expected_state, label="confirmation structural state")
    _constant(
        structural["reason"],
        "ok" if expected_state == "pass" else "fixed-triplet-failed-confirmation-support",
    ...
    for confirmation_item, sealed_raw in zip(
        normalized, sealed_selected, strict=True
    ):
    ...
    if expected_state == "insufficient" and _triplet_meets_requirements(measurements):
        ...
        if per_graph_pass:
            raise P4RunError("insufficient confirmation triplet passes every frozen gate")
    return structural
...
def _validate_matrix_projection(
    ...
    *,
    ...
    _constant([item.get("cell_id") for item in cells], required_ids, label=f"{role} cell ids")
    ...
    for item, axes in zip(cells, expected_axes, strict=True):
        ...
        _constant(
            item["cycle_class"], cycle_class, label=f"{role} cell cycle class"
        )
        _constant(
    ...
    _constant(matrix["worst_oracle_or_null_error_cycles"], worst, label=f"{role} worst error")
    checks = [
    ...
    _constant(matrix["purpose_adjacency_check_count"], 18, label=f"{role} purpose count")
    if len(checks) != 18:
    ...
    selected_edge_order = {
        str(item["family"]): item["edge_fingerprint_sha256"]
        for item in sealed_selected
    }
    ...
    for item, (case_name, cycle_class, family) in zip(
        ...
        _constant(item["fingerprint_equality_required"], False, label="fingerprint equality requirement")
        _constant(item["canonical_adjacency_equal"], True, label="purpose adjacency equality")
        _constant(item["canonical_edge_order_sha256_equal"], True, label="purpose edge order equality")
        _require_sha256(item["field_graph_fingerprint_sha256"], label="field graph fingerprint")
        _require_sha256(item["cycle_graph_fingerprint_sha256"], label="cycle graph fingerprint")
    ...
    spans = [
        ...
        for item in _sequence(
            matrix["graph_family_spans"], label=f"{role} spans"
        )
    ]
    required_span_ids = list(totals_by_span)
    if len(spans) != 6:
        raise P4RunError(f"{role} matrix must carry six spans")
    _constant([item.get("span_id") for item in spans], required_span_ids, label=f"{role} span ids")
    expected_span_axes = list(
        product(
            (name for name, _expected in CROSSED_CASES), CYCLE_CLASS_ORDER
        )
    )
    for item, (case_name, cycle_class) in zip(
        spans, expected_span_axes, strict=True
    ):
        _exact_keys(
            item,
        ...
        _constant(
            item["cycle_class"], cycle_class, label=f"{role} span cycle class"
        )
        totals = totals_by_span[str(item["span_id"])]
        expected_span = max(totals) - min(totals) if totals else None
        _constant(item["graph_family_span_cycles"], expected_span, label=f"{role} span value")
    return matrix
...
def _observation_keys(observation_id: str, state: object) -> set[str]:
    ...
    if control_id == "degree_preserving_rewire":
        return (
            {"observation_id", "state", "degree_preserved", "edge_set_changed", "simple_graph_verified", "symmetric_difference_edge_count", "reason"}
            if state == "insufficient"
            else {"observation_id", "state", "degree_preserved", "edge_set_changed", "simple_graph_verified", "symmetric_difference_edge_count", "removed_edges", "added_edges"}
        )
    ...
    if control_id == "orientation_reversal":
        return {"observation_id", "state", "error_cycles", "tolerance_cycles", "receipt_sha256"}
    ...
    if observation_id == "collapsed_cycleless_phantom|path-graph":
        return {"observation_id", "state", "edge_count", "component_count", "largest_component_vertex_count", "two_core_vertex_count", "cycle_rank", "edge_fingerprint_sha256", "canonical_edges", "state_sha256"}
    ...
    if control_id == "field_only_shuffle":
        return (
            ...
            if state == "fail"
            else {"observation_id", "state", "base_total_cycles", "shuffled_total_cycles", "sign_reversal_error_cycles", "tolerance_cycles"}
        )
...
def _derive_control_raw_state(
    ...
    *,
    ...
    if control_id == "density_warp_confirmation":
        ...
        cells_finite = len(required_cells) == 54 and all(
            item is not None and item["attempt_status"] == "evaluable"
            for item in required_cells
        ...
        return (
            "pass"
            if all(float(value) <= oracle_threshold for value in cell_values if value is not None)
            and all(float(value) <= graph_threshold for value in span_values if value is not None)
            else "fail"
        )
    ...
    if control_id in boolean_fields:
        ...
        for item in observations:
            ...
            if selected is None:
                raise P4RunError(
                    f"{control_id} lacks sealed family edge evidence"
                )
            _constant(
            ...
            if control_id == "joint_vertex_permutation":
                ...
                transformed_vertex_order = array_sha256(
                ...
                base_id_edges = _vertex_id_edges_from_rows(
                ...
                transformed_content = _row_edge_content_sha256(
                    transformed_id_edges
                )
            ...
            _constant(
                item[field], equality, label=f"{control_id} derived equality"
            )
    ...
    if control_id == "collapsed_cycleless_phantom":
        ...
        _require_sha256(path_observation["state_sha256"], label="collapsed state digest")
        _constant(
    ...
    if control_id == "field_only_shuffle":
        ...
        for item in observations:
            if "reason" in item:
                ...
                _constant(item["tolerance_cycles"], 1e-8, label="field shuffle tolerance")
                _constant(error, abs(shuffled + base), label="field shuffle recomputed error")
                expected = "pass" if error is not None and error <= 1e-8 else "fail"
            _constant(item["state"], expected, label="field shuffle observation state")
            states.append(expected)
        return "pass" if len(states) == 18 and all(state == "pass" for state in states) else "fail"
...
def _fold_attempted_controls(
    ...
    *,
    ...
    known = [str(item["control_verdict"]) for item in controls if item["control_id"] in known_ids]
    orientation = [str(item["control_verdict"]) for item in controls if item["control_id"] in orientation_ids]
    other = [str(item["control_verdict"]) for item in controls if item["control_id"] not in known_ids | orientation_ids]
...
def _validate_calibration_algebraic(
    ...
    for key in ("pure_so2_gauge", "orientation_reversal"):
        ...
        if key == "pure_so2_gauge":
            coordinate_error = _nonnegative_finite_scalar(
                item["coordinate_law_error"]
            )
            _constant(
            ...
            expected_state = (
                ...
                if error is None or coordinate_error is None
                    ...
                    "pass"
                    if error <= 1e-8 and coordinate_error <= 1e-8
                    else "fail"
                )
            )
        else:
...
def _calibration_summary(
    ...
    span_values = [
        ...
        for item in _sequence(
            matrix["graph_family_spans"], label="calibration spans"
        )
    ]
    algebraic_values = [
        _nonnegative_finite_scalar(
    ...
    inventory = {
        ...
        "pure_so2_gauge_error_cycles": (
            1 if algebraic_values[0] is not None else 0
        ),
        "orientation_reversal_error_cycles": (
...
def _validate_not_run_result_branch(
    ...
    if state == "invalid":
        ...
        _validate_not_run_controls_reason(
            controls, reason="caught-execution-error"
        )
        return
    ...
    if reason == "no-distinct-three-family-scale-triplet":
        ...
        _constant(result["graph_selection_sealed"], True, label="selector-stop graph seal")
        _constant(result["threshold_decision_sealed"], False, label="selector-stop threshold seal")
    ...
    _constant(result["threshold_decision_sealed"], True, label="post-selector threshold seal")
    ...
    if not bool(summary["finite_complete"]):
        ...
        _constant(result["confirmation_matrix"], None, label="nonfinite confirmation matrix")
        _validate_not_run_controls_reason(
            controls, reason="insufficient-calibration-resolution"
        )
        return

    expected_four = _expected_effective_thresholds(
        summary, include_algebraic=False
    )
    ...
    if not algebraic_resolved or not oracle_cap or not graph_cap:
        ...
        _constant(result["confirmation_accessed"], False, label="calibration gate access")
        _constant(result["confirmation_structural"], None, label="calibration gate structural")
...
def _normalize_result(value: Mapping[str, object]) -> dict[str, object]:
    ...
    control_rows = [
        _mapping(item, label="terminal control") for item in controls
    ]
    ...
    for row in control_rows:
        ...
        if attempted:
            ...
            if control_id == "zero_holonomy_finite_amplitude_null":
                ...
                if type(nuisance["finite"]) is not bool or type(nuisance["passes"]) is not bool:
            ...
            _constant(
                row["control_verdict"], "not_run", label=f"{control_id} verdict"
            )
    ...
    if all(attempted_values):
        _constant(result["confirmation_accessed"], True, label="attempted confirmation access")
        _constant(result["graph_selection_sealed"], True, label="attempted graph seal")
        _constant(result["threshold_decision_sealed"], True, label="attempted threshold seal")
        selector = _validate_selector_projection(result["calibration_selector"])
        _constant(selector["state"], "pass", label="attempted-control selector state")
        selector_selected = _sequence(
            selector["selected"], label="selector selected"
        )
        calibration_matrix = _validate_matrix_projection(
            result["calibration_matrix"],
            role="calibration",
        ...
        if not oracle_cap or not graph_cap:
            raise P4RunError("attempted controls require calibration thresholds within caps")
        _constant(
            result["effective_thresholds"],
            _expected_effective_thresholds(summary, include_algebraic=True),
        ...
        for row in control_rows:
            ...
            _constant(row["raw_state"], derived_raw, label=f"{control_id} derived raw state")
            expected_raw = EXPECTED_RAW_CONTROL_STATES[control_id]
            derived_verdict = (
                "pass"
        ...
        _constant(result["terminal_state"], folded_state, label="terminal attempted fold")
    ...
    *,
    ...
    if execution_terminal == "complete":
        if error is not None or result.get("terminal_state") == "invalid":
            raise P4RunError("complete terminal requires non-invalid result and null error")
...
def prepare_launch(repo_root: Path) -> dict[str, object]:
    ...
    launch = {
        ...
        "coordinates": {
            ...
            "python_bytecode_cache_prefix": str(
                _python_bytecode_cache_prefix(root)
            ),
        },
...
def validate_committed_launch(
    ...
    *,
    ...
    if _tracked_head_bytes(root, REPOSITORY_LAUNCH) != (root / REPOSITORY_LAUNCH).read_bytes():
    ...
    for path in (*_source_paths(protocol), REPOSITORY_LAUNCH):
        _require_regular_head_file(root, path)
    _constant(launch.get("schema_version"), LAUNCH_SCHEMA_VERSION, label="launch schema")
...
def _read_regular_canonical_document(path: Path, *, label: str) -> tuple[bytes, dict[str, object]]:
    metadata = os.lstat(path)
...
def _validate_graph_selection_seal(
    ...
    *,
    ...
    _constant(
        seal["schema_version"],
        "spirallens.p4-graph-selection-seal.v0.1",
...
def _validate_threshold_seal_payload(seal: Mapping[str, object]) -> None:
    ...
    oracle_threshold, oracle_cap = _effective_threshold(oracle_worst, cap=0.05)
    graph_threshold, graph_cap = _effective_threshold(span_worst, cap=0.1)
    _constant(seal["oracle_and_null_cycles"], oracle_threshold, label="threshold oracle value")
    _constant(seal["oracle_and_null_cap_cycles"], 0.05, label="threshold oracle cap")
    _constant(seal["graph_family_span_cycles"], graph_threshold, label="threshold graph value")
...
def _validate_threshold_seal(
    ...
    *,
    ...
    _constant(
        seal["schema_version"],
        "spirallens.p4-threshold-decision-seal.v0.1",
...
def _build_store_manifest(
    ...
    *,
    ...
    return {
        "schema_version": "spirallens.p4-external-store-manifest.v0.1",
        "experiment_id": EXPERIMENT_ID,
...
def _validate_external_bundle_fd(
    ...
    *,
    ...
    _constant(
        ...
        "spirallens.p4-external-store-manifest.v0.1",
        label="external store manifest schema",
    )
    _constant(manifest["experiment_id"], EXPERIMENT_ID, label="manifest experiment")
    _constant(manifest["attempt_exactly_one"], True, label="manifest attempt exactly one")
    _constant(manifest["terminal_at_most_one"], True, label="manifest terminal at most one")
    _constant(manifest["terminal_guaranteed"], False, label="manifest terminal guarantee")
    _constant(
    ...
    _constant(manifest["attempt_sha256"], attempt_sha256, label="manifest attempt digest")
    _constant(manifest["terminal_sha256"], terminal_sha256, label="manifest terminal digest")
    _validate_attempt_document(documents[ATTEMPT_NAME], repo_root=repo_root, launch=launch)
...
def run_official(repo_root: Path) -> dict[str, object]:
    ...
    try:
        ...
        try:
            ...
            def observed_confirmation_access() -> str:
                ...
                _source, digest = _write_stage_document(
                    ...
                    {
                        "schema_version": (
                            "spirallens.p4-confirmation-access-seal.v0.1"
                        ),
                        "experiment_id": EXPERIMENT_ID,
                        "attempt_sha256": attempt_sha256,
                        "launch_sha256": launch_sha256,
            ...
            caught_result = {
                ...
                "threshold_seal_sha256": execution_observation[
                    "threshold_seal_sha256"
                ],
                ...
                "confirmation_structural": None,
                "confirmation_matrix": None,
                "confirmation_accessed": execution_observation[
                    "confirmation_accessed"
                ],
        ...
        try:
            run_process_post = _validated_current_python_process(
                root, mode="--run"
            )
            _constant(
                run_process_post,
        ...
        terminal["python_process_post_execution_observation"] = dict(
            run_process_post
        )
        ...
        if arguments.validate_protocol:
            ...
            response = {
                ...
                "terminal_state": _mapping(
                    terminal["result"], label="terminal result"
                )["terminal_state"],

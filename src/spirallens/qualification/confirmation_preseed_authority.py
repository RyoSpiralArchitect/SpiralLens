...
Strict loading requires complete, non-shallow Git history.  Artifact chronology
is checked across full HEAD-reachable path history: exactly one direct-child
introduction is admitted, and every later path event must descend from it while
retaining the identical blob.  Current source verification separately checks
the anchor and HEAD plus every bounded source-path event on their descendant
ancestry, so an edit followed by a revert or merge resolution cannot restore
live readiness.
...
MAX_D7_ITEM21_HISTORY_COMMITS = 256
MAX_D7_ITEM21_SOURCE_MEMBERS = fused_start.MAX_D7_SOURCE_RUNTIME_MEMBER_COUNT
MAX_D7_ITEM21_SOURCE_MEMBER_BYTES = fused_start.MAX_D7_SOURCE_RUNTIME_MEMBER_BYTES
...
def _repository_root(value: str | Path) -> Path:
    ...
    _require_complete_git_history(root)
    return root


def _require_complete_git_history(root: Path) -> None:
    shallow = _git(root, "rev-parse", "--is-shallow-repository").stdout
    if shallow != b"false\n":
        raise QualificationContractError(
            "item-21 verification requires complete non-shallow Git history"
        )
...
def _commit_sequence(source: bytes, *, label: str) -> tuple[str, ...]:
    try:
        raw_commits = tuple(
            line for line in source.decode("ascii").splitlines() if line
        )
    except UnicodeDecodeError as error:
        raise QualificationContractError(f"{label} is not ASCII") from error
    if (
        not raw_commits
        or len(raw_commits) > MAX_D7_ITEM21_HISTORY_COMMITS
        or len(set(raw_commits)) != len(raw_commits)
    ):
        raise QualificationContractError(
            f"{label} is empty, duplicate, or exceeds its commit cap"
        )
    return tuple(_commit(value, label=f"{label} commit") for value in raw_commits)


def _bounded_path_history(
    root: Path,
    *,
    revision: str,
    ...
    paths = tuple(
        _repository_path(path, label=f"{label} repository path")
        for path in repository_paths
    )
    arguments = ["log", "--full-history"]
    if ancestry_path:
        arguments.append("--ancestry-path")
    arguments.extend(
        (
            f"--max-count={MAX_D7_ITEM21_HISTORY_COMMITS + 1}",
            "--format=%H",
            revision,
            "--",
            *paths,
    ...
    source = _git(root, *arguments).stdout
    if not source:
        return ()
    return _commit_sequence(source, label=label)


def _execution_source_tree(root: Path, commit: str) -> tuple[bytes, ...]:
    tree = _git(
        root,
        "ls-tree",
        "-r",
        "-z",
        "--full-tree",
        _commit(commit, label="execution source history commit"),
        "--",
        *fused_start._SOURCE_PATHS,
    ).stdout
    return tuple(entry for entry in tree.split(b"\0") if entry)


def _require_execution_source_history_continuity(
    root: Path,
    source_commit: str,
) -> None:
    anchor = _commit(source_commit, label="execution source history anchor")
    head = _head(root)
    _require_ancestor(root, anchor, head, label="execution-source anchor-to-HEAD")
    expected_tree = _execution_source_tree(root, anchor)
    events = _bounded_path_history(
        root,
        revision=f"{anchor}..{head}",
        repository_paths=fused_start._SOURCE_PATHS,
        ancestry_path=True,
        label="execution-source descendant path history",
    )
    for commit in dict.fromkeys((anchor, head, *events)):
        if _execution_source_tree(root, commit) != expected_tree:
            raise QualificationContractError(
                "execution-source history changed after the item-21 anchor"
            )
...
def _introduction_commit(
    ...
    *,
    ...
    history = _bounded_path_history(
        root,
        revision="HEAD",
        repository_paths=(path,),
        ancestry_path=False,
        label="artifact full Git history",
    )
    expected_delta = f"A\t{path}\n".encode()
    candidates: list[str] = []
    direct_child_seen = False
    invalid_direct_child_delta = False
    for commit in history:
        ...
        if parent_line != [commit, expected_parent]:
            continue
        ...
        if delta != expected_delta:
            invalid_direct_child_delta = True
            continue
        try:
            candidate_source = _blob(root, commit, path)
        except QualificationContractError:
            continue
        if candidate_source == expected_source:
            candidates.append(commit)
    if len(candidates) != 1:
        if not candidates and invalid_direct_child_delta:
            raise QualificationContractError(
                "item-21 artifact introduction changed more than its one added file"
            )
        if not candidates and not direct_child_seen:
            raise QualificationContractError(
    ...
    introduction_entry = _tree_entry(root, introduction, path)
    if introduction_entry is None:
        raise QualificationContractError("item-21 introduction entry is absent")
    for event in history:
        _require_ancestor(
            root,
            introduction,
            event,
            label="item-21 artifact unique introduction lineage",
        )
        if _tree_entry(root, event, path) != introduction_entry:
            raise QualificationContractError(
                "item-21 artifact changed within reachable full Git history"
            )
...
def _source_inventory(
    ...
    *,
    ...
    if require_current_equality:
        _require_execution_source_history_continuity(root, commit)
    tree = _git(
        root,
        "ls-tree",
        "-r",
    ...
    for raw in (entry for entry in tree.split(b"\0") if entry):
        ...
        if size <= 0 or size > MAX_D7_ITEM21_SOURCE_MEMBER_BYTES:
            raise QualificationContractError(
                "execution source member exceeds its fixed byte cap"
            )
        source = _git(root, "cat-file", "blob", object_id).stdout
        if len(source) != size:
